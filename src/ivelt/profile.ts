// Aggregates a user's PUBLIC ivelt posts into an activity/interest profile.
...
// Yiddish day/month names as they appear in ivelt's date strings
// (e.g. "מאנטאג מאי 25, 2026 2:18 am").
...
/** Best-effort parse of an ivelt date string. Language-independent for the
 *  time (digits + am/pm); Yiddish word maps for day-of-week and month. */
export function parseIveltDate(text: string | null | undefined): ParsedDate {
  ...
  for (const [yi, en] of Object.entries(DOW)) {
    if (t.startsWith(yi)) {
      dayOfWeek = en;
      break;
    }
  ...
  for (const [yi, n] of Object.entries(MONTHS)) {
    if (t.includes(yi)) {
      month = n;
      break;
    }
...
export function summarizePosts(
  ...
  for (const p of posts) {
    ...
    const d = parseIveltDate(p.postedAt);
    if (d.hour24 !== null) {
